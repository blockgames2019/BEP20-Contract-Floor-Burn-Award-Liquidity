...
contract TestToken is Context, IERC20, Ownable {
    ...
    string private _name = 'MUSST TOKEN';
    string private _symbol = 'MUSST';
    uint8 private _decimals = 18;
    uint256 private _totalSupply = 38888 * 10 ** uint256(_decimals);
    ...
    // 流动性合约地址
    function setLiquidityContract(address liquidityContract) public onlyOwner {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (_balances[_burnPool] >= MAX_STOP_FEE_TOTAL) {
            removeAllFee();
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        !_isExcludedFromFee[recipient] &&
        recipient != _exchangePool &&
        (sender == _liquidityContract
        || recipient == _liquidityContract)
        ) {
            ...
            recipientNubmer = tTransferAmount + tLiquidity;
            _balances[recipient] = _balances[recipient].add(recipientNubmer);

            _burnFeeTotal = _burnFeeTotal.add(tBurn);
            _balances[_burnPool] = _balances[_burnPool].add(tBurn);

            emit Transfer(sender, _burnPool, tBurn);
            emit Transfer(sender, recipient, recipientNubmer);
        }
